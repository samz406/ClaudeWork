...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.CLAUDEWORK_ELECTRON_PATH = getElectronNodeRuntimePath();
...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'ClaudeWork Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'ClaudeWork Skill Downloader',
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'clawork-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'clawork-skill-'));
        cleanupPath = tempRoot;
          ...
          if (githubSource) {
            try {
              ...
              if (errno === 'ENOENT' && process.platform === 'win32') {
                throw new Error(
                  'Git executable not found. Please install Git for Windows or reinstall ClaudeWork with bundled PortableGit.'
            ...
            throw new Error('Git executable not found. Please install Git for Windows or reinstall ClaudeWork with bundled PortableGit.');
          } else {
            throw error;
          }
        }
