...
if command -v node > /dev/null 2>&1; then
  NODE_BIN="node"
elif [ -n "${CLAUDEWORK_ELECTRON_PATH:-}" ] && [ -x "${CLAUDEWORK_ELECTRON_PATH}" ]; then
  NODE_BIN="$CLAUDEWORK_ELECTRON_PATH"
  NODE_ENV_PREFIX=("ELECTRON_RUN_AS_NODE=1")
else
  echo '{"success":false,"error":"未找到 Node.js 运行时。请安装 Node.js 或确保 ClaudeWork Electron 可用。"}' >&2
  exit 1
fi
